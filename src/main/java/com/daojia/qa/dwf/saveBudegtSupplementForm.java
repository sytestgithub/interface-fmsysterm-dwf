...
public class saveBudegtSupplementForm  extends BaseTest<T>{
	String method_url="";
	DBUtil db;
	Form form =new Form();
	@BeforeClass
	...
	/*
	    * 保存和提交预算增补单
	    * */
		@Test(dataProvider = "saveBudegtSupplementFormParam", description="保存和提交日常报销单接口")
//	@Test(description="保存和提交日常报销单接口")
		public void saveBudegtSupplementForm1(String name){
			method_url="http://fmsystem205.djtest.cn/v2/formsubmit";
//			method_url="http://fmsystem205.djtest.cn/v2/submit/budegtsupplementform";
			Form form =new Form();
			form.setFormId("YSZB201711270002");//YSZB201711270000
			form.setFormType(9);
			  ...
			  String sql1="select * from  t_form WHERE  form_id='YSZB201711270002'";
	           ...
	           if(form_status1.equals("2")){
	          	 ...
	          	 if(form_statusagins.equals("2")){
			 	 		 ...
			 	 		 String  formstatusupdate="UPDATE t_form  SET form_status=1 WHERE form_id='YSZB201711270002'";
	          	 ...
	          	 Logger.log("订单YSZB201711270002状态非2是"+form_status1);
	           }
			...
			String formOption="save";
			 ...
			 file.setFileAddress("http://www.baidu.com");
				...
				file.setIsPic("0");
				 ...
				 file1.setFileAddress("http://www.baidu.cn");
...
//						form.setFormStatus("1");  没有订单状态
					...
					form.setMoneyType("");
					 ...
					 String sqlofformid=" SELECT * FROM t_form WHERE  form_type=9 AND form_status=2 ORDER BY apply_date DESC";
				    String formId=GetDbMethod.getCostDetailSelctTest(db, sqlofformid, "form_id");
					 form.setFormId(formId);
					  formOption="submit";
						 costDetails.add(cost);
				 }else if(name.equals("completestatusnotcansave")){
					 String sqlofformid=" SELECT * FROM t_form WHERE  form_type=9 AND form_status=9 ORDER BY apply_date DESC";
					 String formId=GetDbMethod.getCostDetailSelctTest(db, sqlofformid, "form_id");
					    form.setFormId(formId);
						  formOption="submit";
							 costDetails.add(cost);
...
//				
					 if(name.equals("savenothaveanyvalue")||name.equals("updatesavehaveall")||name.equals("receiptmoneynotisnull")){
						
						 if(result.get("message").toString().trim().contains("成功")){
							 Assert.assertEquals(result.get("repCode").toString().trim(),"0");
						 }else{
							 Assert.assertEquals(1, 0);
						 }
						
						 getverifaction( name, result, costDetails,fileList,form);
					 }else if(name.equals("moneytypenotisnull")||name.equals("applymoneynotisnull")){
						 if(result.get("message").toString().trim().contains("失败")){
							 Assert.assertEquals(result.get("repCode").toString().trim(),"1");
						 }else{
							 Assert.assertEquals(1, 0);
						 }
					 }else if(name.equals("completestatusnotcansave")||name.equals("processstatusnotcansave")){
...
@DataProvider
  public static Object[][] saveBudegtSupplementFormParam() {
      return new Object[][]{
...
public void getverifaction(String name, JSONObject result,List<CostDetail> costDetails,List<UploadFileDto> fileList,Form form){

	Logger.log("修改成功=========》》》》》》》》》》》》》》》》》》》》》》》");
	String sqlofformid="SELECT * FROM t_form WHERE  form_id='YSZB201711270002'";	
...
//		 // 获取当前申请金额=费用明细总金额
			...
			Logger.log("进入savenothaveanyvalue校验");
		Assert.assertEquals(resultparambacks.get("form_id").toString().trim(),form.getFormId());
...
//		 Assert.assertEquals(resultparambacks.get("payee_name").toString().trim(),"尚英");
		...
		Assert.assertEquals(resultparambacks.get("money_type").toString().trim(),form.getMoneyType());
		 Assert.assertEquals(resultparambacks.get("state").toString().trim(),"0");
			...
			if(costDetails.size()>0||!costDetails.isEmpty()){
				 ...
				 String sqlofcostdetails="SELECT * FROM t_cost_details WHERE cost_form_id='YSZB201711270002'";	
...
//						Assert.assertEquals(resultparambackscostdetails.get("cost_detail_type").toString().trim(),costDetails.get(i).getCostDetailType());
			...
			if(fileList.size()>0||!fileList.isEmpty()){
				 String sqloffileaddress="SELECT * FROM t_file_address WHERE  form_id='YSZB201711270002' ORDER BY id ASC";	
